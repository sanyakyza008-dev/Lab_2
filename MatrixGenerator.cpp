...
int main(int argc, char* argv[]) {
    ...
    if (fs::exists(file_A) && fs::exists(file_B)) {
        std::cout << "[INFO] Файлы matrix_A.bin и matrix_B.bin уже существуют\n";
        std::cout << "Создание пропущено. Можно запускать calculator.exe\n";
        return 0;
    }
    ...
    if (save_matrix(file_A, A) && save_matrix(file_B, B)) {
        ...
        std::cerr << "[ОНИБКА] Не удалось сохранить файлы\n";
    }
